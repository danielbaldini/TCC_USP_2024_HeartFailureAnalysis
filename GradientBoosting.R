...
predito_boosting_train <- predict(glm1,train, type = "response" )

#cut_boost_treino <- testarCutoff(train$DEATH_EVENT, predito_boosting_train)
#cut_boost_treino

predito_boosting_train <- ifelse(predito_boosting_train >=0.56,1,0)
...
predito_boosting_test <- predict(glm1,test, type = "response" )

#cut_boost_teste <- testarCutoff(test$DEATH_EVENT, predito_boosting_test)
#cut_boost_teste

predito_boosting_test <- ifelse(predito_boosting_test>=0.56,1,0)
