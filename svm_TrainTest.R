...
#cut_svm_treino <- testarCutoff(train$DEATH_EVENT, as.numeric((attr(predictedTrain, "probabilities")[,1]), 8))
#cut_svm_treino

predictedTrain <- as.factor(if_else(attr(predictedTrain, "probabilities")[,1] >0.45,1,0))
...
#cut_svm_teste <- testarCutoff(test$DEATH_EVENT, as.numeric((attr(predictedTest, "probabilities")[,1]), 8))
#cut_svm_teste

predictedTest <- as.factor(if_else(attr(predictedTest, "probabilities")[,1] >0.45,1,0))
